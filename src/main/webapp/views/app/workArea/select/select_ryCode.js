...
window.app.selectStaff = function() {
        ...
        table.render({
            ...
            cols: [
                [
                    ...
                    { unresize: true,field: 'code',width:'80%',title: '人员编号',align: 'center'},
    ...
    var operateColumn = function(d) {
        var row = '';
        row += '<a href="javascript:void(0);" class="oc" onclick=selectRyCode(\''+d.id+'\',\''+d.code+'\')>选择</a>';
        return row;
    }
    
    this.queryListCode  = function() {
        var code = $('#ryCode_select').val();
...
var selectRyCode = function(id,code) {
    $('#id').val(id);
    $('#code').val(code);
