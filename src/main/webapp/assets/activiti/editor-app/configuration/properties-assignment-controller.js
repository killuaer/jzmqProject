...
var KisBpmAssignmentCtrl = [ '$scope', '$modal', function($scope, $modal) {
    ...
    var opts = {
        template:  sourcePath+'/'+'editor-app/configuration/properties/assignment-popup.html?version=' + Date.now(),
        scope: $scope
    };
...
$scope.getProjectDataAsync = function(){ 
	$http({ 
		method: 'POST', 
		url: ACTIVITI.CONFIG.contextRoot+'/model/getProjectList' }).then(function successCallback(response) { 
...
$scope.dataWatch = function (){ 
	...
	$scope.$watch('pagingOptions', function (newValue, oldValue) { 
		$scope.getPagedDataAsync($scope.pagingOptions.currentPage, 
				$scope.pagingOptions.pageSize, $scope.selectedProject); 
	},true); 
	//当切换类型时，初始化当前页
...
$scope.getPagedDataAsync = function(pageNum, pageSize, projectId){ 
	...
	if($scope.selectType == 2){ 
		...
		$scope.gridData = response.data.rows; $scope.totalServerItems = response.data.total; 
	}, 
	function errorCallback(response) { 
		// 请求失败执行代码 
		$scope.gridData = []; 
		...
		beforeSelectionChange: function (event) { 
			...
			if($scope.selectType == 0){
				...
				if(!array_contain($scope.assignment.candidateGroups, obj.value)){ 
					$scope.assignment.candidateGroups.push(obj); } } return true; 
		} 
};
$scope.getGroupData = function(data){ 
	...
	$scope.selectCandidate = function () { 
		$scope.selectType = 1; $scope.selectTitle = '选择候选人'; 
	}; //候选组
